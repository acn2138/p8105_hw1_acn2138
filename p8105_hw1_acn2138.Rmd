...
```
Penguins is a tibble with `r ncol(penguins)` variables and `r nrow(penguins)` rows. The variables are: `r names(penguins)`. Mean flipper length is `r mean(pull(penguins, as.numeric(flipper_length_mm)),na.rm=TRUE)`.
